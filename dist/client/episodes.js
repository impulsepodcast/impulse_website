...
function updateQuery(search, tags, page) {
...
function main() {
    const episodes = readJson("episodes-data");
    ...
    const activeTags = new Set((url.searchParams.get("tags") ?? "")
        .split(",")
        .map((tag) => tag.trim())
        .filter(Boolean));
    ...
    function renderPagination(totalItems) {
        ...
        const buttons = [];
        buttons.push(`<button class="pagination__button" type="button" data-page="${currentPage - 1}" ${currentPage === 1 ? "disabled" : ""}>Previous</button>`);
        for (let page = 1; page <= totalPages; page += 1) {
            buttons.push(`<button class="pagination__button${page === currentPage ? " is-active" : ""}" type="button" data-page="${page}" aria-current="${page === currentPage ? "page" : "false"}">${page}</button>`);
        }
        buttons.push(`<button class="pagination__button" type="button" data-page="${currentPage + 1}" ${currentPage === totalPages ? "disabled" : ""}>Next</button>`);
        safePagination.innerHTML = buttons.join("");
    }
    function applyFilters() {
        const search = safeSearchInput.value.trim().toLowerCase();
        const cards = Array.from(safeGrid.querySelectorAll("[data-episode-card]"));
        const matchingCards = [];
